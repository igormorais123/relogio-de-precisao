...
// Fundo em plano fixo atrás da câmera: gradiente radial, névoa por ruído, cor por capítulo e corte diagonal na transição.
const vert = /* glsl */`
varying vec2 vUv;
...
void main(){
  ...
  // Máscara diagonal: 0 = capítulo atual, 1 = próximo. Borda com ruído para não parecer corte de papel.
  ...
  float glow = exp(-r * r * 1.3) * 0.022;
  float mist = fbm(p * 1.4 + vec2(uTime * 0.02, -uTime * 0.015)) * 0.22;
  vec3 col = base * (1.0 + mist * 1.8) + accent * (glow + mist * 0.012);
...
export class Background {
  constructor(scene) {
    this.uniforms = {
      uColorA: { value: new THREE.Color(0x06080b) }, uColorB: { value: new THREE.Color(0x06080b) },
      uAccentA: { value: new THREE.Color(0xd3a94f) }, uAccentB: { value: new THREE.Color(0xd3a94f) },
  ...
  set(colorA, accentA, colorB, accentB, mix) {
    this.uniforms.uColorA.value.setHex(colorA); this.uniforms.uAccentA.value.setHex(accentA)
    this.uniforms.uColorB.value.setHex(colorB); this.uniforms.uAccentB.value.setHex(accentB)

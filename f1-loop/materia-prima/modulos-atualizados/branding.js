...
export async function applyInteiaBranding(model, mechanics) {
  ...
  const crestMap=await new THREE.TextureLoader().loadAsync(`${import.meta.env.BASE_URL}assets/inteia-crest-racing-v1.png`);
  crestMap.colorSpace=THREE.SRGBColorSpace;crestMap.anisotropy=8;
  ...
  const mask=document.createElement('canvas');mask.width=1024;mask.height=1536;
  const mc=mask.getContext('2d');mc.fillStyle='#000';mc.fillRect(0,0,1024,1536);
  ...
  const crestMaterial=new THREE.MeshStandardMaterial({name:'INTEIA | escudo neural racing',map:crestMap,alphaMap:crestAlpha,alphaTest:.5,roughness:.4,metalness:.12,transparent:true,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-4});

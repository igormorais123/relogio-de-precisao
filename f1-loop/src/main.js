...
function paintCopy(){
 ...
 $$('.chapter').forEach((s,i)=>{const r=s.getBoundingClientRect();if(r.bottom<-vh*.5||r.top>vh*1.3)return;
  ...
  const raw=-r.top/Math.max(1,r.height-vh),enter=i===0?1:(i===2||i===3)?smooth((raw-.02)/.08):i===4?smooth((raw+.04)/.08):narrow?smooth((raw+.03)/.03):smooth((raw+.34)/.3),leave=i===5||s.querySelector('.lesson[open]')?0:smooth((raw-.74)/.18);

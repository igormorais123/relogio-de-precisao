...
export async function openEngine({opener=document.activeElement}={}){
 ...
 dialog.innerHTML=`<div class="engine-stage" aria-label="Motor tridimensional. Arraste para girar e use a roda do mouse para aproximar."></div><div class="engine-top"><span class="engine-mark">INTEIA / ANATOMIA DO MOVIMENTO</span><button class="engine-close" type="button">Voltar à aula ↗</button></div><div class="engine-caption"><p class="engine-kicker">01 / O QUE O MOVIMENTO ESCONDE</p><h2 id="engine-title">POR DENTRO<br>DO IMPULSO.</h2><p class="engine-explanation">O conjunto parece uma coisa só. Pare o ciclo para enxergar como cada peça participa.</p></div><p class="engine-status" role="status">Preparando o motor…</p><div class="engine-bottom"><div><div class="engine-controls"><button data-engine="reveal" disabled>Interromper o automático</button><button data-engine="run" disabled>Girar o ciclo</button><button data-engine="reset" disabled>Recompor</button></div><span class="engine-hint">Arraste para olhar de outro ângulo · role para aproximar</span></div><p class="engine-footnote">Motor conceitual didático · movimento desacelerado.<br>Geometria original, sem representar o projeto de uma equipe.<br><a class="engine-source" href="${source}" target="_blank" rel="noopener">Arquitetura V6 turbo / referência F1 ↗</a></p></div>`;
 ...
 function reveal(){openStart=openAmount;speedStart=speed;phase='reveal';phaseTime=0;interacted=false;controls.autoRotate=false;caption('02 / INTERROMPER PARA ENTENDER','PARE O TEMPO.<br>ABRA O SISTEMA.','O virabrequim fica. Os conjuntos se afastam e revelam o que o movimento contínuo escondia.');if(reduced){openAmount=1;speed=0;phase='open';}}
 function run(){phase='running';phaseTime=0;speed=.25;caption('03 / UMA RELAÇÃO VISÍVEL','SEIS PISTÕES.<br>UM MOVIMENTO.','A translação dos pistões vira rotação no virabrequim. Observe a ligação, não apenas o resultado.');}
 function reset(){openStart=openAmount;phase='return';phaseTime=0;interacted=false;caption('01 / O QUE O MOVIMENTO ESCONDE','POR DENTRO<br>DO IMPULSO.','O conjunto parece uma coisa só. Pare o ciclo para enxergar como cada peça participa.');if(reduced){openAmount=0;speed=0;phase='assembled';}}
 ...
 try{
  ...
  if(!reduced){phase='running';speed=.25;}else caption('01 / INSPEÇÃO SEM MOVIMENTO','POR DENTRO<br>DO IMPULSO.','Abra o conjunto ou gire o ciclo quando quiser. A animação automática está desativada.');

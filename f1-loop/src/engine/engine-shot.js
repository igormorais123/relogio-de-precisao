...
const KEYS=[
 ...
 [.24,[1.55,1.30,.15],[0,.42,-.62]],    // over the sidepod, into the bay
 [.30,[1.20,.82,-.30],[0,.40,-.60]],    // 1 block: the section opens on the crank
 [.45,[1.05,.74,-.80],[0,.39,-.64]],
 [.55,[.62,1.30,.05],[0,.54,-.66]],     // 2 intake: from above, where the air enters
 [.67,[.30,1.22,-.22],[0,.55,-.74]],
...
const RANGE=[[0,3],[.2,1.2],[.30,.32],[.45,.32],[.50,.7],[.55,.34],[.67,.34],[.72,.7],[.77,.30],[.90,.30],[1,3]];
...
export function engineShot(progress,mobile=false){
 ...
 let fov=eased(FOV,p);
 // Portrait: the engine sits above the copy, so the lens pulls back and opens instead of cropping the part.
 if(mobile){camera=camera.map((v,i)=>target[i]+(v-target[i])*1.32);fov+=9;}
 return {
  weight:smooth(p/.12),
  camera,target,fov,

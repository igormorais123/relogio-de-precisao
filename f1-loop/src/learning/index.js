...
export function renderLearningMarkup() {
  return `<section class="lr-lab" data-learning-root aria-label="Prática de revisão com fonte"><p class="lr-tag">CASO DIDÁTICO · INTEIRAMENTE FICTÍCIO</p><h3>O teste melhorou. O comunicado está correto?</h3><p>Você revisa um comunicado interno sobre um formulário de atendimento. Compare a fonte com a candidata, registre os erros e decida o que o texto permite comunicar.</p><details data-lr-documents open><summary>Rever fonte e candidata</summary><div class="lr-docs"><article><h4>Fonte · registro do teste</h4><ol>${SOURCE.map(s=>`<li>${esc(s)}</li>`).join('')}</ol></article><article><h4>Candidata · ainda sem revisão</h4><ol>${CLAIMS.map(c=>`<li>${esc(c.text)}</li>`).join('')}</ol></article></div></details><p class="lr-progress" data-lr-progress>O avanço depende das ações do exercício. Não há nota de IA nem aprovação automática.</p>
${stage(0,'Escreva com suas palavras o que fará o comunicado estar pronto e o que não pode piorar. Use como referência a data, os grupos, as medianas e a distinção entre observação, causa e autorização.',written('criterion','Critério do caso','Está pronto quando… Não pode piorar…')+check('self-review','Eu revisei meu critério: outra pessoa consegue conferir o que escrevi na fonte.')+'<p class="lr-detail">O texto será guardado, não avaliado por IA ou por palavras-chave. As próximas etapas usam critérios explícitos do caso; não certificam seu critério livre.</p><section class="lr-self-review" data-lr-self-review><h4>Compare sem nota automática</h4><p data-lr-criterion-comparison>Após guardar, seu critério aparecerá aqui para comparação.</p><p><strong>Critério-modelo:</strong> Cada afirmação deve ter apoio na fonte; preservar 14 de maio de 2026, os dois grupos de 50 pedidos e as medianas de 12 e 9 minutos, sem atribuir causa ou acrescentar adoção ou treinamento não confirmados.</p>'+check('review-source','Outra pessoa consegue conferir meu critério na fonte?')+check('review-data','Meu critério preserva data, grupos e medianas?')+check('review-cause','Meu critério separa observação de causa e de informação sem confirmação?')+'<p>Marque apenas o que você mesmo conferiu. Estas respostas não são avaliadas e não dão uma nota ao seu texto.</p></section>','Guardar meu critério')}
...
${stage(2,'Esta é uma simulação previamente escrita, sem chamada a IA. Guarde o pedido exato, a fonte e a candidata antes de avaliar; a ação cria uma cópia no registro desta sessão.',`<h4>Pedido exato da simulação</h4><blockquote class="lr-request">${esc(SIMULATION_REQUEST)}</blockquote><p>A resposta é a candidata de quatro frases acima. O pedido é adequado, mas a candidata pode descumpri-lo: é isso que você vai conferir.</p>${select('archive-choice','Qual registro você guardará antes de avaliar?', [['corrigida','Pedido e fonte com a candidata já corrigida nos pontos que parecem óbvios.'],['substituir','Pedido e fonte com a nova versão substituindo a candidata anterior.'],['intacta','Pedido exato, fonte e candidata intacta, em registro separado da futura correção.']])}`,'Guardar pedido, fonte e candidata intacta')}
...
export function mountLearning(container, { chapterIndex = 0, onNavigate } = {}) {
  ...
  function draw() {
    ...
    $('[data-lr-criterion]').textContent = state.criterion ? `Seu critério, sem avaliação automática: ${state.criterion}` : '';
    ...
    $('[data-lr-criterion-comparison]').textContent = state.criterion ? `Seu critério, preservado literalmente: ${state.criterion}` : '';
    ...
    $('[data-lr-decision]').textContent = state.decision ? `Decisão: ${label('decision',state.decision)} Motivo: ${label('decision-reason',state.decisionReason)} Sua ressalva, sem avaliação automática: ${state.decisionNote || 'Não registrada.'}` : '';
  }
